...
- **Dead-click detector produces false positives.** Observed during replay-patch smoke test (2026-04-22). Two failure modes: (a) click on a button whose handler throws an error → handler ran, no DOM mutation produced → flagged dead incorrectly; (b) click into an input field → browser focuses the input (cursor, focus ring), but focus/blur are Input events (rrweb source 5), not Mutation events (source 0), and our detector only watches Mutations → flagged dead incorrectly. V3 fix: widen the "reaction" window in `src/extractors/clicks.ts` to also accept rrweb Input events and MouseInteraction focus subtypes as evidence of reaction, not only Mutation events. ~5 lines of change, big signal-quality improvement.
